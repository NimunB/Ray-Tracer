...
int gr_nh_cylinder_cmd(lua_State* L)
{
  ...
  data->node = new GeometryNode(name, new NonhierCylinder(pos, radius));
...
// Create a non-hierarchical sheet node
extern "C"
int gr_nh_sheet_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  data->node = new GeometryNode(name, new NonhierSheet(pos, length, width));
...
int gr_nh_cone_cmd(lua_State* L)
{
  ...
  data->node = new GeometryNode(name, new NonhierCone(pos, radius));
...
int gr_render_cmd(lua_State* L)
{
  ...
  for (int i = 1; i <= light_count; i++) {
	...
	final_Render(root->node, im, eye, view, up, fov, ambient, lights);
    im.savePng( filename );
...
int gr_mirror_material_cmd(lua_State* L)
{
  ...
  double shininess = luaL_checknumber(L, 3);
  double glossiness = luaL_checknumber(L, 4);
  
  data->material = new PhongMaterial(glm::vec3(kd[0], kd[1], kd[2]),
                                     glm::vec3(ks[0], ks[1], ks[2]),
                                     shininess, 1.0, glossiness, true);
...
// Create an Image Textured Material
extern "C"
